...
class Min(ValidationTest):
    ...
    def __init__(self, min, precision=1):
        self.min = min
...
class Max(ValidationTest):
    ...
    def __init__(self, max, precision=1):
        self.max = max
        self.precision = precision
...
class Range(ValidationTest):
    ...
    def __init__(self, min, max, precision=1):
...
class InList(ValidationTest):
    ...
    def __init__(self, list):
        if(len(list) < 2): raise Exception("If the list has minus of 2 elements, why don't make an exact test?")
        self.list = list
...
def verifyFailure(resultCollectParameter, method, *args):
    ...
    except Exception, e: # TODO Exception base exceptionClass=Exception, identificar erro de falha normal
        pass 
#        print(str(e))
...
def _listIsSorted(list, cmp=None, key=None, reverse=False):
    clone = list[:]
    clone.sort(cmp=cmp, key=key, reverse=reverse)
    return clone == list

def assertListIsSorted(list, cmp=None, key=None, reverse=False):
    sorted_list = sorted(list, cmp=cmp, key=key, reverse=reverse)
    if(list != sorted_list):
        raise failureException("List is not sorted. Expected list: %s" % sorted_list)

def assertListIsNotSorted(list, cmp=None, key=None, reverse=False):
    sorted_list = sorted(list, cmp=cmp, key=key, reverse=reverse)
    if(list == sorted_list):
