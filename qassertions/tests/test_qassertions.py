...
class RegExprValidationTests(unittest.TestCase):        
    ...
    def helper_success_values(self, regexpr, list):
        REGEXPR = regexpr
        self.assertEquals(list, RegExpr(REGEXPR).successValues())
        for s in RegExpr(REGEXPR).successValues():
            self.assertTrue(re.match(REGEXPR, s), str(s))
            self.assertTrue(re.search(REGEXPR, s), str(s))

    def helper_failure_values(self, regexpr, list):
        REGEXPR = regexpr
        self.assertEquals(list, RegExpr(REGEXPR).failureValues())
